...
public class HideAudioFlyoutMenuPreference extends SwitchPreference {
    {
        if (SpoofStreamingDataPatch.multiAudioTrackAvailable()) {
            String summaryOn = str("rvx_morphed_hide_player_flyout_menu_audio_track_summary_on");
            String summaryOff = str("rvx_morphed_hide_player_flyout_menu_audio_track_summary_off");
            setSummaryOn(summaryOn);
            setSummaryOff(summaryOff);
        } else { // Audio menu is not available if spoofing to Android client type.
            String summary = str("rvx_morphed_hide_player_flyout_menu_audio_track_not_available");
