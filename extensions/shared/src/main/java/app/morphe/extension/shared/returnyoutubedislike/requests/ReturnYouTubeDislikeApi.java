...
public class ReturnYouTubeDislikeApi {
    ...
    private static void updateRateLimitAndStats(long timeNetworkCallStarted, boolean connectionError, boolean rateLimitHit) {
        ...
        if (connectionError) {
            ...
            if (!lastApiCallFailed && toastOnConnectionError) {
                Utils.showToastLong(str("rvx_morphed_ryd_failure_client_rate_limit_requested"));
            }
            lastApiCallFailed = true;
        } else {
            fetchCallResponseTimeLast = responseTimeOfFetchCall;
    ...
    public static RYDVoteData fetchVotes(String videoId) {
        ...
        try {
            ...
            if (responseCode == HTTP_STATUS_CODE_SUCCESS) {
                ...
                handleConnectionError(str("rvx_morphed_ryd_failure_connection_status_code", responseCode),
                        responseCode, null, Toast.LENGTH_LONG);
            }
            connection.disconnect(); // Something went wrong, might as well disconnect.
        } catch (SocketTimeoutException ex) {
            handleConnectionError((str("rvx_morphed_ryd_failure_connection_timeout")), null, ex, Toast.LENGTH_SHORT);
        } catch (IOException ex) {
            handleConnectionError((str("rvx_morphed_ryd_failure_generic", ex.getMessage())), null, ex, Toast.LENGTH_LONG);
        } catch (Exception ex) {
            // should never happen
    ...
    public static String registerAsNewUser() {
        ...
        try {
            ...
            handleConnectionError(str("rvx_morphed_ryd_failure_generic", "registration failed"), null, ex, Toast.LENGTH_LONG);
        } catch (Exception ex) {
    ...
    private static String confirmRegistration(String userId, String solution) {
        ...
        try {
            ...
            handleConnectionError(str("rvx_morphed_ryd_failure_generic", "confirm registration failed"),
    ...
    public static boolean sendVote(String userId, String videoId, ReturnYouTubeDislike.Vote vote) {
        ...
        try {
            ...
            handleConnectionError(str("rvx_morphed_ryd_failure_generic", "send vote failed"), null, ex, Toast.LENGTH_LONG);
        } catch (Exception ex) {
            // should never happen
    ...
    private static boolean confirmVote(String videoId, String userId, String solution) {
        ...
        try {
            ...
            handleConnectionError(str("rvx_morphed_ryd_failure_generic", "confirm vote failed"),
                    null, ex, Toast.LENGTH_LONG);
        } catch (Exception ex) {
